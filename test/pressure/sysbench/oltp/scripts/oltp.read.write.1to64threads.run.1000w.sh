...
sysbench --test=$testmode --db-driver=mysql --oltp-table-size=10000000 --mysql-db=sysbench1000w --mysql-user=sysbench --mysql-password=sysbench --max-time=${runtime} --max-requests=0 --num-threads=${thread} run >> ./$logname
